...
pub trait WasmExecutor: Send + Sync {
    ...
    fn execute(&self, target: &str, input: &str, args: &[String]) -> Result<Vec<GeneratedFile>, WasmError>;
    /// Executes the target returning the raw stdout bytes (typically for JSON outputs).
    fn execute_to_stdout(&self, target: &str, input: &str, args: &[String]) -> Result<Vec<u8>, WasmError>;
    /// Executes a raw WASI command for the CLI, returning (stdout, stderr).
    fn execute_cli(&self, target: &str, input_dir: Option<&Path>, mount_current_dir: bool, args: &[String]) -> Result<(Vec<u8>, Vec<u8>), WasmError>;
}
...
pub static WASM_EXECUTOR: Lazy<NativeWasmExecutor> = Lazy::new(|| {
...
impl NativeWasmExecutor {
    ...
    fn get_module(&self, wasm_file: &str) -> Result<Module, WasmError> {
        let mut cache = self.module_cache.lock().map_err(|e| WasmError::ExecutionFailed(format!("Mutex lock failed: {}", e)))?;
    ...
    fn run_wasi(&self, target: &str, input_dir: Option<&Path>, mount_current_dir: bool, args: &[String]) -> Result<(Vec<u8>, Vec<u8>), WasmError> {
        ...
        if let Some(dir) = input_dir {
            builder.preopened_dir(dir, "/workspace", DirPerms::all(), FilePerms::all())
                .map_err(|e| WasmError::InvalidConfig(format!("Failed to mount /workspace: {}", e)))?;
        }
        ...
        let instance = linker.instantiate(&mut store, &module)
            .map_err(|e| WasmError::ExecutionFailed(format!("Failed to instantiate module: {}", e)))?;
        ...
        if let Err(err) = start.call(&mut store, ()) {
            ...
            if !msg.contains("exit status 0") {
                ...
                return Err(WasmError::ExecutionFailed(format!("Execution failed: {}\nStderr: {}", err, stderr_str)));
            }
        }
...
impl WasmExecutor for NativeWasmExecutor {
    fn execute(&self, _target: &str, _input: &str, _args: &[String]) -> Result<Vec<GeneratedFile>, WasmError> {
        Ok(vec![])
    }
