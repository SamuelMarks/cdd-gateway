    ...
    match req.method.as_str() {
        ...
        "to_docs_json" => {
            ...
            if is_wasm
                && matches!(
                    target.as_str(),
                    "cdd-java" | "cdd-python" | "cdd-python-all" | "cdd-sh"
                )
            {
            ...
            let output_result = if is_wasm {
                ...
                match tokio::task::spawn_blocking(move || {
                    crate::wasm_executor::WASM_EXECUTOR.execute_to_stdout(&target_clone, &input_clone, &args_clone)
            ...
            match output_result {
                Ok(stdout) => {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&json_str) {
                        Ok(j) => {
                            HttpResponse::Ok().json(RpcResponse::success(j, req.id.clone()))
                        }
