...
async fn test_create_and_find_user() {
    ...
    let github_id = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as i64;

    let user = repo.create_user(Some(github_id), username.clone(), email.clone(), Some("hash".to_string())).await.unwrap();
    assert_eq!(user.username, username);
    assert_eq!(user.email, email);

    let found = repo.find_user_by_username(username.clone()).await.unwrap().unwrap();
...
async fn test_upsert_user() {
    ...
    let username = format!("user_{}", Uuid::new_v4());
    let email = format!("{}@example.com", username);
    let github_id = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as i64 + 1;

    let user1 = repo.upsert_user(github_id, username.clone(), email.clone()).await.unwrap();
    assert_eq!(user1.username, username);

    // Upsert again with new username
    let new_username = format!("user_{}_new", Uuid::new_v4());
    let user2 = repo.upsert_user(github_id, new_username.clone(), email.clone()).await.unwrap();
