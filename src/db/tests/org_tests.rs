...
async fn test_create_and_get_org() {
    ...
    let org = repo.create_organization(Some(rand::random::<i64>().abs()), login.clone(), Some("desc".to_string())).await.unwrap();
...
async fn test_upsert_org() {
    ...
    let org1 = repo.upsert_organization(github_id, login.clone(), Some("desc1".to_string())).await.unwrap();
    assert_eq!(org1.description.unwrap(), "desc1");

    let new_login = format!("org_{}_new", Uuid::new_v4());
    let org2 = repo.upsert_organization(github_id, new_login.clone(), Some("desc2".to_string())).await.unwrap();
...
async fn test_user_organization_link() {
    ...
    let user = repo.create_user(None, username.clone(), email.clone(), None).await.unwrap();

    let link = repo.add_user_to_organization(org.id, user.id, "admin".to_string()).await.unwrap();
    ...
    let link_update = repo.add_user_to_organization(org.id, user.id, "member".to_string()).await.unwrap();
