...
async fn test_create_and_get_repository() {
    ...
    let repository = repo.create_repository(org.id, Some(rand::random::<i64>().abs()), name.clone(), Some("repo desc".to_string())).await.unwrap();
...
async fn test_upsert_repository() {
    ...
    let repo1 = repo.upsert_repository(org.id, github_id, name.clone(), Some("desc1".to_string())).await.unwrap();
    assert_eq!(repo1.description.unwrap(), "desc1");

    let new_name = format!("repo_{}_new", Uuid::new_v4());
    let repo2 = repo.upsert_repository(org.id, github_id, new_name.clone(), Some("desc2".to_string())).await.unwrap();
